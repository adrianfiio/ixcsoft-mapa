...
class MapV07540ContractTests(unittest.TestCase):
    ...
    def test_physical_rack_module_is_loaded_after_v07539(self):
        ...
        new_index = template.index("map-rack-physical-v07540.js")
        self.assertLess(old_index, new_index)
        self.assertIn("map-rack-physical-v07540.css", template)

    def test_rack_is_mounted_by_units_and_locked(self):
        runtime = content("static/js/map-rack-physical-v07540.js")
        css = content("static/css/map-rack-physical-v07540.css")
    ...
    def test_version_is_current(self):
        ...
        release = content("docs/releases/map/map-v0.75.40.md")
        self.assertIn('MAP_VERSION = os.getenv("MAP_VERSION", "0.75.40")', settings)
        self.assertIn('MAP_VERSION: ${MAP_VERSION:-0.75.40}', compose)
        self.assertIn("Mapa | v0.75.40", versions)
        self.assertIn("MAP v0.75.40", release)
