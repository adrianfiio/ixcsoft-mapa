...
class MapV07542ContractTests(unittest.TestCase):
    ...
    def test_versions_are_current(self):
        self.assertIn('MAP_VERSION = os.getenv("MAP_VERSION", "0.75.50")', content("config/settings.py"))
        self.assertIn('MAP_VERSION: ${MAP_VERSION:-0.75.50}', content("docker-compose.yml"))
        self.assertIn("Mapa | v0.75.50", content("VERSIONS.md"))
