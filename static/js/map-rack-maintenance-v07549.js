(function (global) {
    ...
    function installNavigation() {
        if (global.__mapRackNavigationV07549) return;
        global.__mapRackNavigationV07549 = true;
        ...
        global.addEventListener("pointermove", (event) => {
            ...
            event.stopPropagation();
            event.stopImmediatePropagation();
            pan.scroll.classList.add("v07549-is-panning");
        ...
        const finishPan = (event) => {
            ...
            pan.scroll.classList.remove("v07549-is-panning");
            state.pan = null;
        };
    ...
    async function loadUplinks(id, oltId, force = false) {
        const key = `${id}:${oltId}`;
        if (!force && state.uplinks.has(key)) return state.uplinks.get(key);
        const data = await request(uplinkUrl(id, oltId));
        state.uplinks.set(key, data);
        return data;
    }
    ...
    function renderUplinkBank(node, data) {
        ...
        if (!face || !serviceSlots || !data.uplink_slot_count) return;
    ...
    function ensurePhysicalFlow() {
        const root = rootNode();
        if (!root || !isRack(root)) return;
        global.mapRackPhysicalV07542?.refresh?.();
    ...
    function installResizeObserver() {
        ...
        state.resizeObserver = new ResizeObserver(() => {
            global.clearTimeout(state._resizeTimer);
            state._resizeTimer = global.setTimeout(ensurePhysicalFlow, 80);
        });
    }
    ...
    async function enhance() {
        ...
        observeEquipment();
        await Promise.all([
            ...qsa('.master-canvas-node[data-equipment-type="olt"]', root).map((node) => enhanceOlt(node, generation).catch((error) => console.warn("MAP v0.75.49 uplink:", error))),
            ...qsa('.master-canvas-node[data-equipment-type="dio"]', root).map((node) => enhanceDio(node, generation).catch((error) => console.warn("MAP v0.75.49 DIO:", error))),
        ]);
        global.setTimeout(ensurePhysicalFlow, 40);
        global.setTimeout(ensurePhysicalFlow, 220);
    }
    ...
    function scheduleEnhance() {
        ...
        state.frame = global.requestAnimationFrame(() => {
            state.frame = 0;
            enhance().catch((error) => console.error("MAP v0.75.49:", error));
        });
        global.setTimeout(() => enhance().catch(() => {}), 180);
    ...
    global.mapRackMaintenanceV07549 = Object.freeze({
        version: VERSION,
