(function (global) {
    ...
    function readView(canvas = canvasNode()) {
        ...
        return { scale: Number(canvas.dataset.v0741Scale || 1) || 1, tx: Number(canvas.dataset.v07552Tx || canvas.dataset.v07546Tx || 0) || 0, ty: Number(canvas.dataset.v07552Ty || canvas.dataset.v07546Ty || 0) || 0 };
    }
    ...
    function applyView(view, canvas = canvasNode()) {
        ...
        canvas.dataset.v07552Tx = String(view.tx);
        canvas.dataset.v07552Ty = String(view.ty);
        canvas.dataset.v07546Tx = String(view.tx);
        canvas.dataset.v07546Ty = String(view.ty);
    ...
    function installNavigation() {
        if (global.__mapRackNavigationV07552) return;
        global.__mapRackNavigationV07552 = true;
        ...
        global.addEventListener("pointerdown", (event) => {
            ...
            // Interrompe os controladores antigos sem cancelar o clique. Se o
            // ponteiro não se mover, slots vazios continuam recebendo click.
            event.stopPropagation();
            event.stopImmediatePropagation();
        }, true);
        ...
        global.addEventListener("pointermove", (event) => {
            ...
            event.stopPropagation();
            event.stopImmediatePropagation();
            pan.scroll.classList.add("v07552-is-panning");
        ...
        const finishPan = (event) => {
            ...
            pan.scroll.classList.remove("v07552-is-panning");
            state.pan = null;
        };
    ...
    function uplinkUrl(id, oltId) { return `/api/map/v07552/elements/${id}/olt/${oltId}/uplinks/`; }
    ...
    const UPLINK_CONNECTORS = [
        ["rj45", "RJ45"], ["sfp", "SFP"], ["sfp_plus", "SFP+"],
        ["xfp", "XFP"], ["qsfp_plus", "QSFP+"],
    ];
    const UPLINK_SPEEDS = [1, 10, 25, 40, 100];

    function connectorOptions(value) {
        return UPLINK_CONNECTORS.map(([key, label]) => `<option value="${key}" ${key === value ? "selected" : ""}>${label}</option>`).join("");
    }

    function speedOptions(value) {
        return UPLINK_SPEEDS.map((speed) => `<option value="${speed}" ${Number(value) === speed ? "selected" : ""}>${speed} Gbps</option>`).join("");
    }

    function renderUplinkPortFields(dialog, specs) {
        const holder = qs("[data-uplink-port-fields]", dialog);
        const quantity = clamp(Number(qs('[name="port_count"]', dialog)?.value || 1), 1, 32);
        const next = Array.from({ length: quantity }, (_, index) => specs[index] || {
            name: `Uplink ${index + 1}`,
            connector_type: "sfp_plus",
            speed_gbps: 10,
        });
        holder.innerHTML = next.map((spec, index) => `<label data-uplink-port-row>
            <span>Porta ${index + 1}</span>
            <input data-uplink-port-name maxlength="100" value="${escapeHtml(spec.name || `Uplink ${index + 1}`)}">
            <select data-uplink-connector>${connectorOptions(spec.connector_type || "sfp_plus")}</select>
            <select data-uplink-speed>${speedOptions(spec.speed_gbps || 10)}</select>
        </label>`).join("");
    }

    function uplinkSpecs(dialog) {
        return qsa("[data-uplink-port-row]", dialog).map((row, index) => ({
            name: qs("[data-uplink-port-name]", row)?.value || `Uplink ${index + 1}`,
            connector_type: qs("[data-uplink-connector]", row)?.value || "sfp_plus",
            speed_gbps: Number(qs("[data-uplink-speed]", row)?.value || 10),
        }));
    }
    ...
    async function renderUplinkDialog(id, oltId, slotNumber) {
        ...
        const card = slot.card;
        const currentSpecs = card?.ports?.map((port, index) => ({
            name: port.name || port.label || `Uplink ${index + 1}`,
            connector_type: port.connector_type || "sfp_plus",
            speed_gbps: Number(port.speed_gbps || 10),
        })) || [{ name: "Uplink 1", connector_type: "sfp_plus", speed_gbps: 10 }];
        const dialog = ensureUplinkDialog();
        dialog.innerHTML = `<form><header><div><small>PLACA DE UPLINK · U${slotNumber}</small><h2>${card ? "Editar uplink" : "Adicionar uplink"}</h2><p>Modelo, nome, conector e velocidade de cada porta.</p></div><button type="button" data-close>×</button></header><div class="v07549-dialog-body"><div class="v07549-form-grid"><label>Modelo da placa<input name="model" maxlength="120" value="${escapeHtml(card?.model || "")}" placeholder="Ex.: HU1A"></label><label>Quantidade de portas<input name="port_count" type="number" min="1" max="32" value="${currentSpecs.length}"></label></div><section class="v07549-uplink-port-fields"><header><strong>Portas físicas</strong><small>RJ45, SFP, SFP+, XFP e QSFP+ com 1/10/25/40/100 Gbps.</small></header><div data-uplink-port-fields></div></section><p data-status></p></div><footer>${card ? '<button type="button" class="danger" data-remove>Remover placa</button>' : ""}<span></span><button type="button" data-close>Fechar</button><button type="submit" class="primary">Salvar uplink</button></footer></form>`;
        renderUplinkPortFields(dialog, currentSpecs);
        qs('[name="port_count"]', dialog).oninput = () => renderUplinkPortFields(dialog, uplinkSpecs(dialog));
        qsa("[data-close]", dialog).forEach((button) => { button.onclick = () => dialog.close(); });
        qs("form", dialog).onsubmit = async (event) => {
            event.preventDefault();
            const status = qs("[data-status]", dialog);
    ...
    function fallbackUplinkPort(port) {
        ...
        button.type = "button";
        button.className = "master-node-port v07549-uplink-port v07552-uplink-port";
    ...
    function takeUplinkPort(node, port) {
        ...
        button.classList.remove("is-speed-1g", "is-speed-10g", "is-speed-25g", "is-speed-40g", "is-speed-100g");
        button.classList.add("v07549-uplink-port", "v07552-uplink-port", `is-${String(port.connector_type || port.port_type).replaceAll("_", "-")}`);
        button.classList.toggle("is-linked", Boolean(port.linked));
        if (port.linked && port.speed_class) button.classList.add(`is-${port.speed_class}`);
        button.dataset.portRole = "front";
        ...
        button.title = `${port.name || port.label || `Porta ${port.index}`} · ${port.connector_type_label || port.port_type_label} · ${port.speed_gbps || "?"} Gbps${port.linked ? " · ligada" : " · livre"}`;
        const label = qs("span", button);
        if (label) label.textContent = port.name || String(port.index);
        if (port.link_id && port.speed_class) {
            global.mapRackSwitchV07552?.registerLinkSpeed?.(String(port.link_id), port.speed_class);
            document.dispatchEvent(new CustomEvent("map:link-speed-v07552", { detail: { linkId: String(port.link_id), speedClass: port.speed_class } }));
        }
        return button;
    }
    ...
    function renderUplinkBank(node, data) {
        ...
        const signature = JSON.stringify({
            ...
            slots: (data.uplink_slots || []).map((slot) => ({
                ...
                ports: (slot.card?.ports || []).map((port) => [port.id, port.name, port.connector_type, port.speed_gbps, Boolean(port.linked)]),
            })),
        });
        qsa(".v07547-uplink-bank, .v07549-uplink-bank").forEach((item) => {
            if (item.closest('.master-canvas-node[data-equipment-type="olt"]') === node && item.dataset.generatedBy !== "v07552") item.remove();
        });
        const canonical = qs('.v07549-uplink-bank[data-generated-by="v07552"]', node);
        if (state.renderedUplinks.get(node) === signature && canonical) return;
        canonical?.remove();
        ...
        bank.dataset.generatedBy = "v07552";
        bank.innerHTML = `<header><div><strong>UPLINK</strong><small>Clique no slot vazio; botão direito edita a placa.</small></div><span>${data.uplink_slot_count}</span></header><div class="v07549-uplink-slots"></div>`;
        ...
        (data.uplink_slots || []).forEach((slot) => {
            ...
            if (slot.empty) {
                ...
                cell.innerHTML = `<header><div><strong>U${slot.slot} · ${escapeHtml(slot.card.model)}</strong><small>${slot.card.ports.length} porta(s)</small></div></header><div class="v07549-uplink-ports v07552-uplink-ports"></div>`;
    ...
    function ensurePhysicalFlow() {
        const root = rootNode();
        if (!root || !isRack(root)) return;
        if (qs(".is-dragging-v07542", root)) return;
    ...
    function scheduleEnhance(delay = 40) {
        ...
        state.enhanceTimer = global.setTimeout(() => {
            ...
            enhance().catch((error) => console.error("MAP v0.75.52:", error));
        }, delay);
    }

    function touchesEquipment(node) {
        if (!(node instanceof Element)) return false;
        if (node.dataset.generatedBy === "v07552") return false;
        if (node.matches?.(".v07547-uplink-bank, .v07549-uplink-bank")) return true;
    ...
    global.mapRackMaintenanceV07552 = Object.freeze({
        version: VERSION,
