(function (global) {
    ...
    function installCableSummary(node) {
        ...
        if (!id) return;
        node.classList.add("map-rack-cable-compact-v07537");
        let summary = qs("[data-rack-cable-summary-v07537]", node);
        if (!summary) {
            summary = document.createElement("div");
            summary.dataset.rackCableSummaryV07537 = "1";
            summary.className = "map-rack-cable-summary-v07537";
            summary.innerHTML = `
                <span><strong>${cableFiberCount(node)} fibras</strong><small>Arraste o ponto até um DIO</small></span>
                <button type="button" data-rack-cable-anchor-v07537="${id}" title="Vincular este cabo a um DIO" aria-label="Vincular cabo ao DIO"><i></i></button>`;
            node.appendChild(summary);
            const anchor = qs("[data-rack-cable-anchor-v07537]", summary);
    ...
    function installCompactOlt(node) {
        node.classList.add("map-olt-compact-v07537");
    ...
    function installDio(node) {
        ...
        node.classList.add("map-dio-matrix-ready-v07537");
        qsa('[data-port-role="rear"], .dio-rear', node).forEach((port) => {
            port.classList.add("map-dio-port-hidden-v07537");
            port.setAttribute("aria-hidden", "true");
            port.tabIndex = -1;
        });
        let target = qs("[data-dio-cable-target-v07537]", node);
        if (!target) {
            target = document.createElement("button");
            target.type = "button";
            target.dataset.dioCableTargetV07537 = String(id);
            target.className = "map-dio-cable-target-v07537";
            target.innerHTML = '<i></i><span><strong>CABOS</strong><small data-dio-linked-count-v07537>0 vinculados</small></span>';
            target.title = "Arraste um cabo até aqui ou abra a matriz de fusões";
        ...
        if (header && !qs("[data-open-dio-fusions-v07537]", header)) {
            const button = document.createElement("button");
            button.type = "button";
            button.dataset.openDioFusionsV07537 = String(id);
            button.className = "map-open-dio-fusions-v07537";
    ...
    function syncAssociationData(elementId, equipmentId, data) {
        ...
        qsa(`.master-canvas-node[data-equipment-node="${equipmentId}"]`).forEach((node) => {
            ...
            const count = qs("[data-dio-linked-count-v07537]", node);
    ...
    function dragOverlay() {
        let svg = qs("#map-rack-cable-drag-v07537");
        if (!svg) {
            svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
            svg.id = "map-rack-cable-drag-v07537";
    ...
    function startCableDrag(event, cableNode, anchor) {
        ...
        state.drag = {
            ...
            cableNode,
            anchor,
            start: center(anchor.getBoundingClientRect()),
        };
        document.body.classList.add("map-cable-linking-v07537");
    ...
    function handleDragMove(event) {
        ...
        updateDragLine(event.clientX, event.clientY);
        qsa(".map-dio-cable-target-v07537.is-drop-target").forEach((item) => item.classList.remove("is-drop-target"));
        const target = document.elementsFromPoint(event.clientX, event.clientY)
            .map((item) => item.closest?.("[data-dio-cable-target-v07537], .master-canvas-node[data-equipment-type='dio']"))
            .find(Boolean);
    ...
    function cancelCableDrag() {
        state.drag = null;
        document.body.classList.remove("map-cable-linking-v07537");
        qs("#map-rack-cable-drag-v07537")?.remove();
        qsa(".map-dio-cable-target-v07537.is-drop-target").forEach((item) => item.classList.remove("is-drop-target"));
    ...
    function updateDragLine(x, y) {
        ...
        const dx = Math.max(60, Math.abs(x - start.x) * 0.42);
        const direction = x >= start.x ? 1 : -1;
        qs("path", svg).setAttribute(
            "d",
            `M ${start.x} ${start.y} C ${start.x + dx * direction} ${start.y}, ${x - dx * direction} ${y}, ${x} ${y}`,
        );
    }
    ...
    function associationSvg() {
        ...
        let svg = qs(".map-rack-association-links-v07537", panel);
        if (!svg) {
            svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
            svg.classList.add("map-rack-association-links-v07537");
            svg.setAttribute("aria-hidden", "true");
    ...
    function drawAssociations() {
        ...
        qsa('.master-canvas-node[data-equipment-type="dio"]', root).forEach((dioNode) => {
            ...
            const target = qs("[data-dio-cable-target-v07537]", dioNode);
            if (!target) return;
            ...
            linked.forEach((linkedCableId) => {
                ...
                const anchor = cableNode && qs("[data-rack-cable-anchor-v07537]", cableNode);
                if (!anchor) return;
    ...
    function ensureWorkspace() {
        ...
        root.className = "map-dio-fusion-workspace-v07537";
        root.innerHTML = `
            <section role="dialog" aria-modal="true" aria-labelledby="map-dio-fusion-title-v07537">
                <header>
                    <div><span>MATRIZ DE FUSÃO</span><h2 id="map-dio-fusion-title-v07537">DIO</h2><p>Clique numa fibra e depois numa porta. A fusão é gravada imediatamente.</p></div>
                    <div class="map-dio-fusion-header-actions-v07537">
                        <button type="button" data-dio-auto-fuse-v07537>⚡ Auto fusão</button>
                        <button type="button" data-dio-close-v07537 aria-label="Fechar">×</button>
                    </div>
                </header>
                <div class="map-dio-fusion-status-v07537" data-dio-status-v07537></div>
                <main data-dio-body-v07537></main>
            </section>
            <div class="map-dio-port-menu-v07537" data-dio-port-menu-v07537 hidden></div>`;
    ...
    async function openWorkspace(elementId, equipmentId) {
        ...
        document.body.classList.add("map-dio-fusion-open-v07537");
    ...
    function closeWorkspace() {
        ...
        document.body.classList.remove("map-dio-fusion-open-v07537");
        closePortMenu();
        enhance();
    }

    function setWorkspaceStatus(message, error = false) {
        const node = qs("[data-dio-status-v07537]", state.workspace || document);
        if (!node) return;
    ...
    function renderWorkspace() {
        ...
        qs("#map-dio-fusion-title-v07537", state.workspace).textContent = `${state.data.dio.name} · ${state.data.dio.capacity} portas`;
        const body = qs("[data-dio-body-v07537]", state.workspace);
        body.innerHTML = `
            <section class="map-dio-cables-v07537">
                <header><strong>CABOS VINCULADOS</strong><span>${state.data.linked_cables.length}</span><button type="button" data-dio-attach-menu-v07537 ${state.data.available_cables.length ? "" : "disabled"}>+ Vincular</button></header>
                <div>${renderCables()}</div>
            </section>
            <section class="map-dio-selection-v07537">
                <header><strong>SELEÇÃO RÁPIDA</strong></header>
                ${renderSelection()}
            </section>
            <section class="map-dio-ports-v07537">
    ...
    function renderCables() {
        if (!state.data.linked_cables.length) {
            return '<p class="map-dio-empty-v07537">Nenhum cabo vinculado. Arraste um cabo no Rack até o DIO ou use “Vincular”.</p>';
        }
        return state.data.linked_cables.map((cable) => {
            ...
            return `<article class="map-dio-cable-card-v07537 ${active ? "is-active" : ""}" data-dio-cable-card-v07537="${cable.id}">
                <header><button type="button" data-dio-select-cable-v07537="${cable.id}"><span>⌁</span><strong>${escapeHtml(cable.name)}</strong><small>${cable.fiber_count} FO · ${used}/${cable.fibers.length}</small></button><button type="button" data-dio-detach-cable-v07537="${cable.id}" title="Desvincular cabo">×</button></header>
                <div class="map-dio-tubes-v07537">${fiberGroups(cable).map(([tubeNumber, fibers]) => `<div class="map-dio-tube-v07537"><b style="--tube-color:${escapeHtml(fibers[0]?.tube_color_hex || "#64748b")}">T${tubeNumber}</b><div>${fibers.map((fiber) => `<button type="button" data-dio-fiber-v07537="${fiber.id}" class="${fiber.fusion ? "is-fused" : ""} ${Number(fiber.id) === Number(state.selectedFiberId) ? "is-selected" : ""}" style="--fiber-color:${escapeHtml(fiber.color_hex || "#94a3b8")}" title="Fibra ${fiber.number} · ${escapeHtml(fiber.color_name)}${fiber.fusion ? ` · DIO ${fiber.fusion.port_number}` : ""}"><i></i><span>${fiber.number}</span></button>`).join("")}</div></div>`).join("")}</div>
            </article>`;
        }).join("");
    }
    ...
    function renderSelection() {
        ...
        return `<div class="map-dio-selection-card-v07537">
        ...
        <p class="map-dio-selection-help-v07537">A segunda seleção cria a fusão. Para romper, clique com o botão direito ou dê dois cliques numa porta ocupada.</p>
    ...
    async function handleWorkspaceClick(event) {
        ...
        closePortMenu();
        if (event.target.closest("[data-dio-close-v07537]")) return closeWorkspace();
        const cableButton = event.target.closest("[data-dio-select-cable-v07537]");
        if (cableButton) {
        ...
        const fiberButton = event.target.closest("[data-dio-fiber-v07537]");
        if (fiberButton) {
            const fiberId = Number(fiberButton.dataset.dioFiberV07537);
            const cableCard = fiberButton.closest("[data-dio-cable-card-v07537]");
        ...
        const portButton = event.target.closest("[data-dio-port-v07537]");
        if (portButton) {
        ...
        if (event.target.closest("[data-dio-auto-fuse-v07537]")) return autoFuse();
        const detach = event.target.closest("[data-dio-detach-cable-v07537]");
        if (detach) return detachCable(Number(detach.dataset.dioDetachCableV07537));
        if (event.target.closest("[data-dio-attach-menu-v07537]")) return attachCableDialog();
    }
    ...
    function openPortMenu(x, y, port) {
        const menu = qs("[data-dio-port-menu-v07537]", state.workspace);
        menu.innerHTML = `<button type="button" data-dio-break-fusion-v07537="${port.fusion.id}">Romper fusão</button><small>${escapeHtml(port.fusion.cable)} F${port.fusion.fiber_number} → DIO ${port.number}</small>`;
        menu.hidden = false;
        menu.style.left = `${Math.min(x, global.innerWidth - 230)}px`;
        menu.style.top = `${Math.min(y, global.innerHeight - 110)}px`;
        const button = qs("[data-dio-break-fusion-v07537]", menu);
        button.onclick = () => {
            closePortMenu();
    ...
    function closePortMenu() {
        const menu = qs("[data-dio-port-menu-v07537]", state.workspace || document);
