(function (global) {
    ...
    function takePortButton(node, port, existing = null) {
        ...
        button.classList.add("v07551-switch-port");
    ...
    function renderSwitchFace(node, data) {
        ...
        if (state.rendered.get(node) === signature && qs(".v07551-switch-face", node)) {
            registerLinkSpeeds(data);
            applyLinkColors();
            return;
        }

        const oldFace = qs(".v07551-switch-face", node);
        ...
        face.className = "v07551-switch-face";
        face.dataset.portCount = String(data.ports.length);
        face.dataset.generatedBy = "v07551";
        face.innerHTML = `<header><div><strong>${escapeHtml(data.equipment.vendor || "Switch")} ${escapeHtml(data.equipment.model || "")}</strong><small>${data.ports.length} porta(s) · conectores e velocidades individuais</small></div>${canEdit() ? `<button type="button" data-v07551-switch-edit="${data.equipment.id}">Editar portas</button>` : ""}</header><div class="v07551-switch-ports"></div>`;
        const holder = qs(".v07551-switch-ports", face);
        if (data.ports.length === 16) holder.classList.add("is-16-ports");
        data.ports.forEach((port) => {
    ...
    async function enhanceSwitchNode(node, generation, force) {
        ...
        try {
            ...
            node.dataset.switchErrorV07551 = error.message;
            const face = qs(".v07551-switch-face", node);
    ...
    function scheduleEnhance(force = false, delay = 35) {
        ...
        state.enhanceTimer = global.setTimeout(() => {
            ...
            enhanceAll(nextForce).catch((error) => console.error("MAP v0.75.51:", error));
        }, delay);
    }
    ...
    function enhanceCreateForms() {
        ...
        let row = qs("[data-v07551-switch-count]", fields);
        if (!row) {
            row = document.createElement("label");
            row.dataset.v07551SwitchCount = "1";
    ...
    function ensureDialog() {
        ...
        dialog.id = "map-switch-editor-v07551";
        dialog.className = "v07551-switch-dialog";
    ...
    function editorRows(data) {
        return data.ports.map((port, index) => `<tr data-v07551-port-row="${port.id}">
            ...
            <td><span class="v07551-link-state ${port.linked ? "is-linked" : ""} ${port.linked ? portSpeedClass(port.effective_speed_gbps) : ""}">${port.linked ? `${port.effective_speed_gbps || "?"}G · ${escapeHtml(port.remote_equipment || "ligada")}` : "Livre"}</span></td>
        </tr>`).join("");
    }
    ...
    function installEditorEvents(dialog, containerId, switchId) {
        ...
        qs("[data-apply-range]", dialog).onclick = () => {
            ...
            qsa("[data-v07551-port-row]", dialog).forEach((row, index) => {
        ...
        qs("form", dialog).onsubmit = async (event) => {
            ...
            const ports = qsa("[data-v07551-port-row]", dialog).map((row, index) => ({
                id: Number(row.dataset.v07551PortRow),
                order: index + 1,
    ...
    function renderEditor(dialog, containerId, switchId, data) {
        ...
        dialog.innerHTML = `<form><header><div><small>SWITCH · MAP v${VERSION}</small><h2>${escapeHtml(data.equipment.name)}</h2><p>Edite nome, conector e velocidade sem apagar ligações existentes.</p></div><button type="button" data-close aria-label="Fechar">×</button></header><section class="v07551-bulk-editor"><strong>Aplicar em intervalo</strong><label>Da porta<input name="range_start" type="number" min="1" max="${lastPort}" value="1"></label><label>Até<input name="range_end" type="number" min="1" max="${lastPort}" value="${lastPort}"></label><label>Conector<select name="range_connector">${connectorOptions("rj45")}</select></label><label>Velocidade<select name="range_speed">${speedOptions(1)}</select></label><button type="button" data-apply-range>Aplicar</button></section><div class="v07551-port-table-wrap"><table><thead><tr><th>#</th><th>Nome da porta</th><th>Conector</th><th>Velocidade</th><th>Ligação</th></tr></thead><tbody>${editorRows(data)}</tbody></table></div><p data-status></p><footer><button type="button" data-close>Fechar</button><button type="submit" class="primary">Salvar portas</button></footer></form>`;
    ...
    function switchIdFromEditButton(button) {
        if (button.dataset.v07551SwitchEdit) return Number(button.dataset.v07551SwitchEdit);
    ...
    function installCaptureHandlers() {
        global.addEventListener("click", (event) => {
            const edit = event.target instanceof Element
                ? event.target.closest("[data-v07551-switch-edit], [data-node-edit], [data-edit-equipment]")
                : null;
    ...
    global.mapRackSwitchV07551 = Object.freeze({
        version: VERSION,
